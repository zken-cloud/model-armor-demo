...
function updateBotMessage(text, source, redactedImage = null, responseImages = []) {
    ...
    if (loadingMessage) {
        ...
        // Images the model generated, after response-side screening/redaction.
        (responseImages || []).forEach((dataUrl, i) => {
            const figure = document.createElement('figure');
            figure.className = 'redacted-figure';
            const img = document.createElement('img');
            img.src = dataUrl;
            img.alt = `Image ${i + 1} generated by the model`;
            img.className = 'redacted-image response-image';
            const caption = document.createElement('figcaption');
            caption.textContent = 'Image generated by the model, as screened by the response template';
            figure.appendChild(img);
            figure.appendChild(caption);
